...
begin
	...
	if CurPageId = wpSelectProgramGroup then begin
		if NOT PrefsPageCreated then begin;
			...
			ContainerBox := TNewCheckListBox.create(PrefsPage);
			ContainerBox.Top := DebugEditBox.Top + DebugEditBox.Height + ScaleY(12);
			ContainerBox.Left := -4;
			ContainerBox.Width := PrefsPage.SurfaceWidth;
			ContainerBox.BorderStyle := bsNone;
			ContainerBox.ParentColor := True;
			ContainerBox.Parent := PrefsPage.Surface;
			ContainerBox.ShowLines := True;
			ContainerBox.AddCheckBox(LangRead('APPEND_EXT_LABEL'), '', 0, False, True, False, False, nil);

			GlobalPrefsCheckBox := TCheckBox.Create(PrefsPage);
			GlobalPrefsCheckBox.Caption := '';
			GlobalPrefsCheckBox.Checked := False;
			GlobalPrefsCheckBox.Parent := ContainerBox;
			...
			AppendExtCheckBox.Parent := ContainerBox;
			...
			HistoryCheckBox.Parent := ContainerBox;
			...
			RemoveDupeCheckBox.Parent := ContainerBox;
			...
			RemoveTempCheckBox.Parent := ContainerBox;
			...
			WarnExecuteCheckBox.Parent := ContainerBox;

			TestCheckBox := TCheckBox.Create(PrefsPage);
			TestCheckBox.Top := WarnExecuteCheckBox.Top + WarnExecuteCheckBox.Height + ScaleY(5);
			TestCheckBox.Width := PrefsPage.SurfaceWidth;
			TestCheckBox.Caption := LangRead('WARN_EXECUTE_LABEL');
			TestCheckBox.Checked := True;
			TestCheckBox.Parent := ContainerBox;
